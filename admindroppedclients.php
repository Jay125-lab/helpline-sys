...
if (isset($_SESSION['ID'])&&isset($_SESSION['email'])) {
  # code...
    $id=$_SESSION['ID'];
    $asignclient="SELECT `ID`, `fname`, `lname`, `phone`, `email` FROM `signupcl` WHERE `ID` IN (SELECT `cl_ID` FROM `clienttherapist` WHERE `assignmentStatus`='dropped'); ";
          ...
          <div class="col">
            <nav aria-label="breadcrumb" class="bg-light rounded-3 p-3 mb-4">
              <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item cl"><a href="admindroppedclients.php" class="active">Assign dropped clients therapist</a></li>
            ...
            <?php } ?>
                ...
                <?php
                    ...
                    <table class="table caption-top">
                        <caption>List of pending Clients who do have dropped their assigned therapist</caption>
                        <thead>
                            <tr>
                            <th scope="col">#</th>
                        ...
                        <tbody>
                            ...
                            foreach ($assignclientres as $client) {
                                ...
                                $therapistquery="SELECT `thID`, `fname`, `lname`, `education`, `age`, `residence`, `gender`, `phoneNo`, `email` FROM `thinfo` WHERE `status`='verified' AND `thID` NOT IN(SELECT `th_ID` FROM `clienttherapist` WHERE `cl_ID`='$clientid'); ";
                            ...
                            <td>
                            <button type="submit" class="btn btn-outline-primary" id="btn" name="submit" value="update" onmouseover="hoverbtn(this)" onmouseout="normalbtn(this)">Assign</button>
                            </td>
                            </form>
                            </tr>
                            <?php
